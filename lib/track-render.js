...
TrackRender.prototype.update = function () {
  ...
  var baseIndex = this.track.getSegmentIndexByZ(z) - 1;
  this.projectSegments(z, baseIndex);
  this.drawSegments(baseIndex);
}

TrackRender.prototype.projectSegments = function (z, baseIndex) {
  var t = this.createTopographyControl(z, baseIndex);
  for (var i = 0; i < this.track.segments.length; i++) {
    var s = this.getNextSegment(baseIndex, i);
    var draw = !this.isSegmentNotValidToDraw(z, i);
    s.projection = this.segmentProjection.project(s, t, baseIndex, z, draw);
    this.adjustTopographyValues(t, s);
  }
}

TrackRender.prototype.drawSegments = function (baseIndex) {
  for (var i = 0; i < this.track.segments.length; i++) {
    var j = this.track.segments.length - i;
    var s = this.getNextSegment(baseIndex, j);
