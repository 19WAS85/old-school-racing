...
TrackRender.prototype.update = function () {
  ...
  _(this.raceRender.render.config.drawSegments).times(function (i) {
    var segment = this.getNextSegment(i);
    if (!segment) return;
...
TrackRender.prototype.getNextSegment = function (i) {
  var playerZ = this.player.position.z - this.raceRender.config.cameraDistance;
  var baseSegment = parseInt(playerZ / this.track.segmentLength);
