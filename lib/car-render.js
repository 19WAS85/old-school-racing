...
CarRender.prototype.getOutOfTrackFactor = function (segment) {
  ...
  var firstBound = segment.projection.bounds[0] - carBounds[0];
  var secondBound = carBounds[1] - segment.projection.bounds[2];
...
CarRender.prototype.getRelativePosition = function (segment) {
  var xMod = this.createTopographycMod(segment.projection.curve.start, segment.projection.curve.end) + this.createCarNoise();
  var yMod = this.createTopographycMod(segment.projection.hill.start, segment.projection.hill.end) + this.createCarNoise();
