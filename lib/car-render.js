...
CarRender.prototype.update = function (segment) {
  ...
  this.sprite.z = this.raceRender.race.track.getRelativeZ(this.car.position.z);
...
CarRender.prototype.getRelativePosition = function (segment) {
  var xMod = this.createCurveMod(segment) + this.createCarNoise();
  var carZ = this.car.position.z - this.player.position.z;
  var relativeZ = this.raceRender.race.track.getRelativeZ(carZ);
  return {
    x: this.player.position.x - this.car.position.x + xMod,
    y: this.car.position.y - this.player.position.y + this.createCarNoise(),
    z: relativeZ + this.createCarNoise()
  };
}

CarRender.prototype.createCurveMod = function (segment) {
  if (this.car.position.x == this.player.position.x) return 0;
  var start = segment.projection.startCurve;
  var end = segment.projection.endCurve;
  var segmentLength = this.raceRender.race.track.segmentLength;
